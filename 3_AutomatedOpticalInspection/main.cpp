...
Mat calculateLightPattern(Mat img)
{
    ...
    ///To estimate the background image, we are going to use a blur with a large kernel size applied to our input image.
...
{
    Mat aux;
    /// if method is normalization, Division
    if(method==1)
    {
...
{
    /// Using command line parser class
    CommandLineParser parser(argc, argv, keys);
    
    parser.about("Automated Optical Inspection");
    /// If requires help show, Note that there are no default values for "help" and "timestamp "so we can check their presence using the has() method
    if (parser.has("help"))
    {
        parser.printMessage();
        return 0;
